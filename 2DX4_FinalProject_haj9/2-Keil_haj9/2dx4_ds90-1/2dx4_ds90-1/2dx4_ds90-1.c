...
//Distance status
void PortN0_Init(void){ //Onboard LED
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R12;                 //activate the clock for Port N
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R12) == 0){};        //allow time for clock to stabilize 
	GPIO_PORTN_DIR_R = 0b00000001;       								    // make port N LED output
  GPIO_PORTN_DEN_R = 0b00000001;
	return;
}

//Displacement status
void PortL1_Init(void){ //External LED
	...
	GPIO_PORTL_DIR_R = 0b00000010;       								    // make port L LED output
  GPIO_PORTL_DEN_R = 0b00000010;
	return;
}

void PortM_Init(void){ //Button column - input
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;                 //activate the clock for Port M
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R11) == 0){};        //allow time for clock to stabilize 
	GPIO_PORTM_DIR_R = 0b00000000;      								    // make PM0 an input, PM0 is reading if the button is pressed or not 
  GPIO_PORTM_DEN_R = 0b00000001; 													//Digitally enable PM0
	return;
} //column scans (outputs from the Button)

void PortE_Init(void){ //Button row - output
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R4;                 //activate the clock for Port E
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R4) == 0){};        //allow time for clock to stabilize 
	GPIO_PORTE_DIR_R = 0b00000001;       								    // make PE0 an output 
  GPIO_PORTE_DEN_R = 0b00000001;
	return;
} //row input signal (inputs from the Button)
...
void Stepper(){
	...
	uint32_t delay = 2; //minimum delay = 2ms
		while(count<8){ //1 inner rotation
...
int main(void) {
  ...
  uint8_t dataReady;
	...
	while(1){
		GPIO_PORTE_DATA_R = 0b00000000; //set port E data
